...
const DoctorChat = () => {
  ...
  return (
    <div className="flex-1 flex overflow-hidden bg-[#F6FAFF]">
      {/* Sidebar: Doctor List */}
                  ...
                  <div key={msg._id} className={`flex gap-3 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                    ...
                    <div className={`max-w-[70%] flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                      <div className={`px-5 py-3 rounded-2xl text-sm leading-relaxed ${
                         isMe ? 'bg-[#06b6d4] text-white rounded-tr-sm' : 'bg-white border border-slate-200 text-slate-800 rounded-tl-sm shadow-sm'
                      }`}>
                         {msg.text}
                      </div>
